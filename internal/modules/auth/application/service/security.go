...
import (
	...
	authresp "erg.ninja/internal/modules/auth/api/response"
...
func (s *AuthService) failedLoginCount(ctx context.Context, tenantID, email, ip string) (int64, error) {
	...
	if count, ok := s.cachedFailedLoginCount(ctx, tenantID, email, ip); ok {
		return count, nil
	}
	count, err := s.repo.CountFailedLoginAttempts(ctx, repository.FailedLoginAttemptCountParams{
		TenantID:       tenantID,
		EmailHash:      emailHash,
		IP:             ip,
	...
	if err != nil {
		if s.shouldIgnoreLoginAttemptStoreError(err) {
			s.log.WarnContext(ctx).Err(err).Msg("auth.service: login-attempt store unavailable in development; continuing without DB-backed lockout count")
			return 0, nil
		}
		return 0, err
	}
...
func (s *AuthService) cachedFailedLoginCount(ctx context.Context, tenantID, email, ip string) (int64, bool) {
	if s.redis == nil {
		return 0, false
	}
	keys := make([]string, 0, 2)
	if normalizeEmailForSecurity(email) != "" {
		keys = append(keys, failedEmailKey(tenantID, email))
	}
	if strings.TrimSpace(ip) != "" {
		keys = append(keys, failedIPKey(ip))
	}
...
func (s *AuthService) shouldIgnoreLoginAttemptStoreError(err error) bool {
	if err == nil || s == nil || s.cfg == nil || strings.EqualFold(s.cfg.App.Env, "production") {
		return false
	}
	msg := strings.ToLower(err.Error())
	return strings.Contains(msg, "auth_login_attempts") && strings.Contains(msg, "sqlstate 42p01")
}
...
func (s *AuthService) isIPBlocked(ctx context.Context, ip string) (bool, error) {
	...
	if s.cfg != nil && !strings.EqualFold(s.cfg.App.Env, "production") && isPrivateOrLoopbackIP(ip) {
		return false, nil
	}
