...
func (r *Repository) ListPosts(ctx context.Context, q ListPostsQuery) ([]*entities.Post, int64, error) {
	...
	if q.Deleted {
		query = query.Unscoped().Where("posts.deleted_at IS NOT NULL")
	} else {
		query = query.Where("posts.deleted_at IS NULL")
	}
	if q.Status != "" {
		query = query.Where("posts.status = ?", q.Status)
	}
	if q.CategoryID != "" {
		query = query.Where("posts.category_id = ?", q.CategoryID)
	}
	...
	if q.Search != "" {
		...
		query = query.Where(
			"LOWER(posts.title) LIKE ? OR LOWER(posts.excerpt) LIKE ? OR LOWER(posts.content) LIKE ? OR LOWER(posts.keywords) LIKE ?",
...
// MovePostsToCategory reassigns posts from one category to another.
func (r *Repository) MovePostsToCategory(ctx context.Context, fromCategoryID string, toCategoryID string) (int64, error) {
	if err := r.ensureDB(); err != nil {
		return 0, err
	}
	...
	if res.Error != nil {
		return 0, fmt.Errorf("posts.repo.MovePostsToCategory: %w", res.Error)
	}
