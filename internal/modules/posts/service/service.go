...
import (
	...
	"erg.ninja/internal/modules/posts/repository"
	"erg.ninja/pkg/cache"
...
func (s *Service) SeedDefaultCategories(ctx context.Context) error {
	...
	for _, data := range defaultCategories {
		...
		if cat != nil {
			// Update icon if it doesn't exist (similar to nestjs logic)
			if cat.Icon == "" && data.Icon != "" {
				if err := s.repo.UpdateCategory(ctx, cat.ID.Hex(), bson.M{"icon": data.Icon}); err != nil {
					s.log.WarnContext(ctx).Err(err).Str("slug", data.Slug).Msg("posts: failed to update icon")
				}
			}
			continue
		}
...
func (s *Service) UploadImage(ctx context.Context, filename string, contentType string, body []byte) (string, error) {
	...
	url, err := s.r2.UploadRaw(ctx, body, "posts/images", path, contentType)
	if err != nil {
