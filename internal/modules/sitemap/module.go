...
import (
	...
	sitemapcontroller "erg.ninja/internal/modules/sitemap/api/controller"
...
type Module struct {
	deps Deps
	ctrl *sitemapcontroller.Controller
	svc  *sitemapservice.Service
}

// NewModule creates a new sitemap module.
func NewModule(deps Deps) *Module {
	svc := sitemapservice.NewService(deps.GORMClient, deps.Log)
	ctrl := sitemapcontroller.NewController(svc, deps.Log, deps.Cfg)
