...
import (
	...
	"erg.ninja/internal/modules/access_control/application/service"
...
func (c *Controller) RegisterRoutes(r *gin.Engine) {
	...
	api.Use(c.requirePermission(policy.PermissionRBACRoleRead))
	...
	roleCreate.Use(c.requirePermission(policy.PermissionRBACRoleCreate))
	roleCreate.POST("/roles", c.CreateRole)

	// Role management (update)
	roleUpdate := api.Group("")
	roleUpdate.Use(c.requirePermission(policy.PermissionRBACRoleUpdate))
	roleUpdate.PUT("/roles/:id", c.UpdateRole)
	roleUpdate.PATCH("/roles/:id", c.UpdateRole)

	// Role management (delete)
	roleDelete := api.Group("")
	roleDelete.Use(c.requirePermission(policy.PermissionRBACRoleDelete))
	...
	userMgmt := api.Group("")
	userMgmt.Use(c.requirePermission(policy.PermissionRBACRoleAssign))
	...
	userRead := api.Group("")
	userRead.Use(c.requirePermission(policy.PermissionRBACPermissionRead), c.requirePermission(policy.PermissionRBACRoleRead))
...
func hasPermission(claims *auth.JWTClaims, required string) bool {
	return policy.Decide(policy.Request{
		Subject:  policy.SubjectFromClaims(claims),
		Resource: policy.Resource{Permission: required},
	}).Allowed
}
