...
import (
	...
	"erg.ninja/internal/modules/access_control/api/dto"
	"erg.ninja/internal/modules/access_control/domain/entity"
	"erg.ninja/internal/modules/access_control/domain/policy"
	"erg.ninja/internal/modules/access_control/infrastructure/repository"
...
func (s *Service) GetEffectivePermissions(ctx context.Context, userID string) (*dto.EffectivePermissionsResponse, error) {
	...
	rolePermList, err := s.repo.ListPermissionNamesByRoleRefs(ctx, roleRefs)
	if err != nil {
		return nil, fmt.Errorf("access_control.GetEffectivePermissions.ListPermissionNamesByRoleRefs: %w", err)
	}
	rolePerms := make(map[string]bool, len(rolePermList))
	for _, p := range rolePermList {
		rolePerms[p] = true
	}
...
func (s *Service) GetFeatureConfig(ctx context.Context, userID string) (*dto.FeatureConfigResponse, error) {
	...
	features := map[string]bool{
		...
		"can_manage_roles":    permissionAllowed(permSet, policy.PermissionRBACRoleRead) || permSet["roles.read"],
		"can_view_audit_logs": permissionAllowed(permSet, policy.PermissionAuditRead) || permSet["system.logs"],
		"can_manage_settings": permissionAllowed(permSet, policy.PermissionRBACAll) || permSet["system.settings"],
	}
...
func deriveFeatureAccess(permSet map[string]bool) dto.FeatureAccessResponse {
	...
	modules := []featureModule{
		...
		{
			prefixes:         []string{"users.", "roles.", "rbac."},
			sidebar:          []string{"users", "roles"},
			dashboardWidgets: []string{"user-stats"},
			quickActions:     []string{"create-user"},
		},
		{
			prefixes:         []string{"system.", "api-keys.", "audit.", "menus.", "pages.", "hoclieu.", "lms.", "elearning."},
	...
	isAdmin := permSet["system.settings"] || permissionAllowed(permSet, policy.PermissionRBACAll)
	var sidebar []string
...
func permissionAllowed(permSet map[string]bool, required string) bool {
	for perm := range permSet {
		if policy.PermissionMatches(perm, required) {
			return true
		}
	}
	return false
