...
// ListPermissionNamesByRoleRefs returns the union of permissions for role names or legacy role IDs.
func (r *Repository) ListPermissionNamesByRoleRefs(ctx context.Context, roleRefs []string) ([]string, error) {
	if err := r.ensureDB(); err != nil {
		return nil, err
	}
	if len(roleRefs) == 0 {
	...
	names := make([]string, 0, len(roleRefs))
	ids := make([]string, 0, len(roleRefs))
	for _, roleRef := range uniqueStrings(roleRefs) {
		roleRef = strings.TrimSpace(roleRef)
		if roleRef == "" {
			continue
		}
		if _, ok := database.ParseObjectID(roleRef); ok {
			ids = append(ids, roleRef)
			continue
		}
		names = append(names, roleRef)
	}
	...
	switch {
	case len(names) > 0 && len(ids) > 0:
		query = query.Where("roles.name IN ? OR roles.id IN ?", names, ids)
	case len(names) > 0:
		query = query.Where("roles.name IN ?", names)
	default:
		query = query.Where("roles.id IN ?", ids)
	}
	if err := query.Order("permissions.name ASC").Scan(&rows).Error; err != nil {
		return nil, fmt.Errorf("access_control.repo.ListPermissionNamesByRoleRefs: %w", err)
	}
...
func (r *Repository) SeedDefaultData(ctx context.Context) error {
	...
	defaultRoles = append(defaultRoles, enterpriseSeedRoles(defaultPerms)...)

	if present, err := r.defaultDataPresent(ctx, defaultPerms, defaultGroups, defaultRoles); err != nil {
		return fmt.Errorf("access_control.repo.SeedDefaultData.preflight: %w", err)
	} else if present {
		r.log.Debug().
			Int("permissions", len(defaultPerms)).
			Int("roles", len(defaultRoles)).
			Msg("access_control: default data already present; seed skipped")
		return nil
	}
...
func (r *Repository) defaultDataPresent(ctx context.Context, perms []entities.Permission, groups []entities.PermissionGroup, roles []entities.Role) (bool, error) {
	...
	if ok, err := r.hasAllPermissions(ctx, permissionNames); err != nil || !ok {
		return ok, err
	}
	...
	if ok, err := r.hasAllPermissionGroups(ctx, groupNames); err != nil || !ok {
		return ok, err
	}
	...
	if ok, err := r.hasAllRoles(ctx, roleNames); err != nil || !ok {
		return ok, err
	}

	if ok, err := r.hasRolePermissionCoverage(ctx, roles); err != nil || !ok {
		return ok, err
	}
	return true, nil
}

func (r *Repository) hasAllPermissions(ctx context.Context, names []string) (bool, error) {
	if len(names) == 0 {
		return true, nil
	}
	var count int64
	if err := r.db.WithContext(ctx).Model(&postgrescore.ACPermission{}).Where("name IN ?", names).Count(&count).Error; err != nil {
		return false, err
	}
	return count >= int64(len(names)), nil
}

func (r *Repository) hasAllPermissionGroups(ctx context.Context, names []string) (bool, error) {
	if len(names) == 0 {
		return true, nil
	}
	var count int64
	if err := r.db.WithContext(ctx).Model(&postgrescore.ACPermissionGroup{}).Where("name IN ?", names).Count(&count).Error; err != nil {
		return false, err
	}
	return count >= int64(len(names)), nil
}

func (r *Repository) hasAllRoles(ctx context.Context, names []string) (bool, error) {
	if len(names) == 0 {
		return true, nil
	}
	var count int64
	if err := r.db.WithContext(ctx).Model(&postgrescore.ACRole{}).Where("name IN ?", names).Count(&count).Error; err != nil {
		return false, err
	}
	return count >= int64(len(names)), nil
}

func (r *Repository) hasRolePermissionCoverage(ctx context.Context, roles []entities.Role) (bool, error) {
	roleNames := make([]string, 0, len(roles))
	permissionNames := make([]string, 0)
	expected := make(map[string]map[string]struct{}, len(roles))
	for _, role := range roles {
		perms := uniqueStrings(role.Permissions)
		if len(perms) == 0 {
			continue
		}
		roleNames = append(roleNames, role.Name)
		expected[role.Name] = make(map[string]struct{}, len(perms))
		for _, permissionName := range perms {
			expected[role.Name][permissionName] = struct{}{}
	...
	err := r.db.WithContext(ctx).
		...
		Joins("JOIN permissions ON permissions.id = role_permissions.permission_id").
		Where("roles.name IN ? AND permissions.name IN ?", roleNames, permissionNames).
		Scan(&rows).Error
	if err != nil {
		return false, err
	}
	...
	for _, row := range rows {
		if present[row.RoleName] == nil {
			present[row.RoleName] = make(map[string]struct{})
		}
		present[row.RoleName][row.PermissionName] = struct{}{}
	}
	for roleName, perms := range expected {
		for permissionName := range perms {
			if _, ok := present[roleName][permissionName]; !ok {
				return false, nil
			}
		}
	}
...
func defaultPermissions() []entities.Permission {
	...
	return appendEnterprisePermissions(perms)
}

func defaultPermissionGroups() []entities.PermissionGroup {
	groups := []entities.PermissionGroup{
	...
	return appendEnterprisePermissionGroups(groups)
}

func appendEnterprisePermissions(perms []entities.Permission) []entities.Permission {
	seen := make(map[string]struct{}, len(perms))
	for _, perm := range perms {
		seen[perm.Name] = struct{}{}
	}
	for _, def := range policy.EnterprisePermissionDefinitions() {
...
func appendEnterprisePermissionGroups(groups []entities.PermissionGroup) []entities.PermissionGroup {
	seen := make(map[string]struct{}, len(groups))
	for _, group := range groups {
		seen[group.Name] = struct{}{}
	}
	nextOrder := len(groups) + 1
	for _, group := range []entities.PermissionGroup{
		{ID: database.NewID(), Name: "hoclieu", Label: "HocLieu", Order: nextOrder},
		{ID: database.NewID(), Name: "lms", Label: "LMS", Order: nextOrder + 1},
		{ID: database.NewID(), Name: "elearning", Label: "Elearning", Order: nextOrder + 2},
		{ID: database.NewID(), Name: "rbac", Label: "RBAC", Order: nextOrder + 3},
	} {
...
func enterpriseSeedRoles(perms []entities.Permission) []entities.Role {
	permissionNames := make([]string, 0, len(perms))
	for _, perm := range perms {
		permissionNames = append(permissionNames, perm.Name)
	}
	...
	defs := policy.EnterpriseRoleDefinitions()
	roles := make([]entities.Role, 0, len(defs))
	for _, def := range defs {
		roles = append(roles, entities.Role{
			ID:          database.NewID(),
			Name:        def.Name,
			Description: def.Description,
			Permissions: expandSeedRolePermissions(def.Permissions, permissionNames),
...
func expandSeedRolePermissions(rolePermissions []string, allPermissionNames []string) []string {
	for _, permissionName := range rolePermissions {
		if permissionName == policy.PermissionAll {
			return append([]string(nil), allPermissionNames...)
		}
	}
	return uniqueStrings(rolePermissions)
}
