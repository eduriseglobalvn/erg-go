...
import (
	...
	trendingentities "erg.ninja/internal/modules/trending/domain/entity"
...
type RefreshResult struct {
	Topics      []*trendingentities.TrendingTopic
	News        []*trendingentities.NewsArticle
	Snapshots   []*trendingentities.TrendingSnapshot
	GeneratedAt time.Time
}

func NewService(repo *trendingrepo.Repository, cache *trendingcache.RedisCache, log *logger.Logger, bus *event.EventBus, cfg config.TrendingConfig) *Service {
	if log == nil {
		log = logger.NoOp()
	}
...
func (s *Service) LatestSnapshotTime(ctx context.Context) (time.Time, error) {
	return s.repo.LatestSnapshotTime(ctx)
}
...
func (s *Service) Refresh(ctx context.Context) (*RefreshResult, error) {
	...
	snapshot := &trendingentities.TrendingSnapshot{
	...
	return &RefreshResult{
		Topics:      topics,
		News:        articles,
		Snapshots:   []*trendingentities.TrendingSnapshot{snapshot},
		GeneratedAt: generatedAt,
	}, nil
}

func (s *Service) ListTopics(ctx context.Context, limit int64) ([]trendingdto.TopicResponse, error) {
	var topics []*trendingentities.TrendingTopic
...
func (s *Service) GetTopic(ctx context.Context, slug string) (*trendingdto.TopicResponse, error) {
...
func (s *Service) ListNews(ctx context.Context, limit int64) ([]trendingdto.NewsArticleResponse, error) {
	var articles []*trendingentities.NewsArticle
...
func (s *Service) ListHistory(ctx context.Context, limit int64) ([]trendingdto.SnapshotResponse, error) {
	snapshots, err := s.repo.ListSnapshots(ctx, limit)
	if err != nil {
		return nil, err
	}
	out := make([]trendingdto.SnapshotResponse, 0, len(snapshots))
	for _, snapshot := range snapshots {
		out = append(out, trendingdto.SnapshotResponse{
...
func (s *Service) GetStats(ctx context.Context) (*trendingdto.TrendingStats, error) {
	...
	return &trendingdto.TrendingStats{
		TotalTopics:     total,
		ActiveTopics:    active,
		AvgScore:        avg,
...
func (s *Service) aggregateSeeds(seeds []trendingrepo.SeedDocument, generatedAt time.Time) ([]*trendingentities.TrendingTopic, []*trendingentities.NewsArticle) {
	topicsBySlug := map[string]*trendingentities.TrendingTopic{}
	articles := make([]*trendingentities.NewsArticle, 0, len(seeds))
	...
	for _, seed := range seeds {
		...
		for i, name := range names {
			...
			if !ok {
				topic = &trendingentities.TrendingTopic{
					Topic:           name,
					Slug:            slug,
		...
		articles = append(articles, &trendingentities.NewsArticle{
			Topic:          names[0],
	...
	topics := make([]*trendingentities.TrendingTopic, 0, len(topicsBySlug))
...
func topicsToDTO(items []*trendingentities.TrendingTopic, limit int64) []trendingdto.TopicResponse {
	if limit > 0 && int(limit) < len(items) {
		items = items[:limit]
	}
	out := make([]trendingdto.TopicResponse, 0, len(items))
...
func topicToDTO(item *trendingentities.TrendingTopic) trendingdto.TopicResponse {
	return trendingdto.TopicResponse{
		ID:              item.ID,
		Keyword:         item.Topic,
...
func newsToDTO(items []*trendingentities.NewsArticle, limit int64) []trendingdto.NewsArticleResponse {
	if limit > 0 && int(limit) < len(items) {
		items = items[:limit]
	}
	out := make([]trendingdto.NewsArticleResponse, 0, len(items))
	for _, item := range items {
		out = append(out, trendingdto.NewsArticleResponse{
			Topic:          item.Topic,
...
func pickTopicNames(seed trendingrepo.SeedDocument) []string {
...
func scoreSeed(seed trendingrepo.SeedDocument, topicRank int) float64 {
...
func collectURLs(topics []*trendingentities.TrendingTopic) []string {
	var urls []string
	for _, topic := range topics {
...
func topicNames(topics []*trendingentities.TrendingTopic) []string {
