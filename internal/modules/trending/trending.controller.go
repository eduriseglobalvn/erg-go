...
func NewController(svc *Service, mongo *database.MongoClient, redis *cache.RedisClient, log *logger.Logger, jwtVal *auth.JWTValidator) *Controller {
...
func (c *Controller) Healthz(ctx *gin.Context) {
	lastRefresh, err := c.svc.repo.LatestSnapshotTime(ctx.Request.Context())
	if err != nil {
...
func (c *Controller) Refresh(ctx *gin.Context) {
	...
	c.writeJSON(ctx, http.StatusAccepted, dto.RefreshResponse{
		Status:      "refreshed",
