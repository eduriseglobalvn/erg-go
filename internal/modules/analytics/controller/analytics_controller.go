...
import (
	...
	"erg.ninja/internal/modules/analytics/service"
...
func NewController(svc *service.Service, log *logger.Logger, cfg *config.Config) *Controller {
...
func (c *Controller) RegisterRoutes(r *gin.Engine) {
	api := r.Group("/api/insight")
	{
		...
		protected.Use(c.permissionMiddleware("system.logs"))
		{
			protected.GET("/stats", c.GetStats)
			protected.GET("/overview", c.GetOverview)
			protected.GET("/posts/summary", c.GetPostSummary)
			protected.GET("/top-content", c.GetTopContent)
			protected.GET("/traffic-sources", c.GetTrafficSources)
			protected.GET("/export", c.Export)
			protected.GET("/insights", c.GetInsights)
			protected.GET("/user-journey/:sessionId", c.GetUserJourney)
			protected.GET("/sessions/:sessionId", c.GetSession)
			protected.GET("/firebase/stats", c.GetFirebaseSyncStats)
		}
	}
}
...
func (c *Controller) permissionMiddleware(permission string) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		ctx.Next()
	}
}
