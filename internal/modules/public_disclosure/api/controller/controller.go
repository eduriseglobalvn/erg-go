...
import (
	...
	docDto "erg.ninja/internal/modules/documents/api/dto"
...
func (c *Controller) Create(ctx *gin.Context) {
	tenantID := tenant.FromContext(ctx.Request.Context())
	ctx.Request.Body = http.MaxBytesReader(ctx.Writer, ctx.Request.Body, storage.MaxRequestBytes(storage.UploadKindDocument, 10<<20))
