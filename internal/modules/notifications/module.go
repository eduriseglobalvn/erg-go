...
import (
	...
	notificationscontroller "erg.ninja/internal/modules/notifications/api/controller"
...
type Module struct {
	...
	digest   *notificationsservice.DigestService
	consumer *notificationsservice.EventConsumer
	ctrl     *notificationscontroller.Controller
}
...
func (m *Module) Setup() error {
	...
	// Core service.
	svc := notificationsservice.NewService(repo, m.deps.Bus,
		notificationsservice.WithNotificationLogger(m.deps.Log),
		notificationsservice.WithMaxRetries(3),
	)

	// Providers — skip nil providers if credentials are not configured.
	if discord := notificationsservice.MustDiscordProvider(m.deps.Cfg.Discord.WebhookURL, m.deps.Log); discord != nil {
		svc.RegisterProviders(discord)
	}
	if telegram := notificationsservice.MustTelegramProvider(m.deps.Cfg.Telegram.BotToken, m.deps.Log); telegram != nil {
		svc.RegisterProviders(telegram)
	}
	if whatsapp := notificationsservice.MustWhatsAppProvider(
	...
	if email := notificationsservice.MustEmailProvider(
	...
	// Digest service.
	digestSvc := notificationsservice.NewDigestService(repo, svc,
		notificationsservice.WithDigestLogger(m.deps.Log),
	)
	digestCfg := notificationsservice.DigestConfig{
		MaxItems:      20,
	...
	// Event consumer.
	consumer := notificationsservice.NewEventConsumer(svc, m.deps.Bus,
		notificationsservice.WithEventConsumerLogger(m.deps.Log),
	)
	...
	m.ctrl = notificationscontroller.NewController(
		svc, repo,
		toDiscord(discord),
		toTelegram(telegram),
		toWhatsApp(whatsapp),
...
func toDiscord(p notificationsservice.NotifierProvider) *providers.DiscordProvider {
...
func toTelegram(p notificationsservice.NotifierProvider) *providers.TelegramProvider {
...
func toWhatsApp(p notificationsservice.NotifierProvider) *providers.WhatsAppProvider {
...
func toEmail(p notificationsservice.NotifierProvider) *providers.EmailProvider {
