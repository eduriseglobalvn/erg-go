...
import (
	...
	"erg.ninja/internal/modules/notifications/dto"
...
func (c *Controller) List(ctx *gin.Context) {
	...
	responses := dto.ToResponses(items)

	c.writeJSON(ctx, http.StatusOK, dto.NotificationListResponse{
		Data:        responses,
		Items:       responses,
		Total:       total,
		Limit:       limit,
...
func (c *Controller) Get(ctx *gin.Context) {
	...
	c.writeJSON(ctx, http.StatusOK, dto.ToResponse(n))
}
...
func (c *Controller) TestDiscord(ctx *gin.Context) {
	var req dto.ChannelTestRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		c.writeError(ctx, http.StatusBadRequest, "INVALID_BODY", err.Error())
		return
	}
