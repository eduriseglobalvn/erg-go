...
// RedirectType represents HTTP redirect status codes.
...
// SchemaType represents supported JSON-LD schema types.
type SchemaType string

const (
	SchemaFAQ      SchemaType = "FAQPage"
	SchemaArticle  SchemaType = "Article"
	SchemaCourse   SchemaType = "Course"
	SchemaVideo    SchemaType = "VideoObject"
	SchemaSoftware SchemaType = "SoftwareApplication"
)
...
func NewService(repo *Repository, ai AIService, log *logger.Logger) *Service {
...
func (s *Service) GetPerformance(ctx context.Context, period string) (*PerformanceResponse, error) {
	...
	cursor, err := s.repo.gscData.Find(ctx, bson.M{"date": bson.M{"$gte": since}})
	if err != nil {
		return nil, fmt.Errorf("seo.GetPerformance find: %w", err)
	}
	defer cursor.Close(ctx)
	var records []*GSCData
	if err := cursor.All(ctx, &records); err != nil {
		return nil, fmt.Errorf("seo.GetPerformance decode: %w", err)
	}
