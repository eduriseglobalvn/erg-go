...
type Module struct {
	...
	ctrl    *Controller
	rssCtrl *RSSController
	blCtrl  *BlacklistController
}
...
func (m *Module) Setup() error {
	...
	// Service.
	m.svc = NewService(m.repo, fetcher,
		m.deps.Log,
		m.deps.Bus,
		WithCrawlerLogger(m.deps.Log),
	)

	// Controllers.
	m.ctrl = NewController(m.svc, m.repo, m.deps.Log)
	m.rssCtrl = NewRSSController(m.repo, m.deps.Queue, m.deps.Log)
	m.blCtrl = NewBlacklistController(m.repo, m.deps.Log)
