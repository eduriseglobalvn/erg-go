...
func (s *Service) CreateQuestion(ctx context.Context, tenantID string, actor Actor, req CreateQuestionRequestDTO) (QuestionResponseDTO, error) {
	...
	kind, err := questionKindFromDTO(req.Kind, req.Type)
	if err != nil {
		return QuestionResponseDTO{}, err
	}
	...
	q := &Question{TenantID: tenantID, Scope: dtoToScope(req.Scope), SubjectID: subjectID, LevelID: levelID, TopicID: topicID, Type: kind, Stem: req.Stem, Choices: choicesFromDTO(req.Choices), Answer: req.Answer, Metadata: req.Metadata}
...
func (s *Service) UpdateQuestion(ctx context.Context, tenantID string, actor Actor, id string, req UpdateQuestionRequestDTO) (QuestionResponseDTO, error) {
	...
	if req.Kind != nil || req.Type != nil {
		...
		kind, err := normalizeQuestionKind(kindValue)
		if err != nil {
			return QuestionResponseDTO{}, err
		}
		update["type"] = kind
...
func (s *Service) PublishQuiz(ctx context.Context, tenantID, id string) (PublishQuizResponseDTO, error) {
	...
	s.publishQuizPublishedAudit(ctx, tenantID, *quiz, version, hash)
	return PublishQuizResponseDTO{Version: version, PackageHash: hash}, nil
}
...
func (s *Service) QuizPackage(ctx context.Context, tenantID, id string) (QuizPackageResponseDTO, error) {
	...
	if quiz == nil {
		return QuizPackageResponseDTO{}, errNotFound
	}
	return quizPackageFromQuiz(*quiz), nil
}
...
func questionFilter(req QuestionListRequestDTO) (bson.M, error) {
	...
	if req.Type != "" {
		kind, err := normalizeQuestionKind(req.Type)
		if err != nil {
			return nil, err
		}
		filter["type"] = kind
...
func questionToDTO(q Question) QuestionResponseDTO {
	...
	return QuestionResponseDTO{ID: q.ID.Hex(), Scope: scopeToDTO(q.Scope), SubjectID: q.SubjectID.Hex(), LevelID: q.LevelID.Hex(), TopicID: topicID, Kind: q.Type, Type: q.Type, Stem: q.Stem, Choices: choicesToDTO(q.Choices), Answer: q.Answer, Metadata: q.Metadata, Status: q.Status, CreatedAt: q.CreatedAt, UpdatedAt: q.UpdatedAt}
}
...
func quizToDTO(q Quiz) QuizResponseDTO {
	return QuizResponseDTO{ID: q.ID.Hex(), Scope: scopeToDTO(q.Scope), Title: q.Title, Kind: q.Kind, SubjectID: q.SubjectID.Hex(), LevelID: q.LevelID.Hex(), TopicIDs: objectIDsToHex(q.TopicIDs), QuestionIDs: objectIDsToHex(q.QuestionIDs), Status: q.Status, Version: q.Version, PackageHash: q.PackageHash, CreatedAt: q.CreatedAt, UpdatedAt: q.UpdatedAt}
}
